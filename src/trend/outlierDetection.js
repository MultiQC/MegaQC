...
function yupToMessage(yup) {
    return value => {
        return yup.validate(value).then(() => {
...
export default function OutlierDetection(outlierType = 'outlierType', outlierThreshold = 'outlierThreshold') {
    let thresholdName, showThreshold;
    const [typeField, typeMeta, typeHelpers] = useField({
        name: outlierType,
        validate: yupToMessage(Yup.string().oneOf(['z', 'none']).label('Outlier Detection Method')),
        type: 'select',
