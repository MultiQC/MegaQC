...
$(function() {
  ...
  $("body").on("change", ".new-filter-type select", function(e) {
    ...
    var val_e = $(this)
      .closest("tr")
      .find(".new-filter-value");
    button_e.prop("disabled", false);
    key_e.html(
      '<select class="form-control form-control-chosen-required" required data-placeholder="[ please select key ]"><option></option></select>'
    );
    cmp_e.html(
      '<select class="form-control form-control-chosen-required" required data-placeholder="[ please select comparison ]"><option></option></select>'
    );
    val_e.html('<input class="form-control" type="text">');
    switch ($(this).val()) {
      case "timedelta":
        key_e.html(
          '<input type="text" readonly class="form-control-plaintext" value="Dynamic dates">'
        );
        cmp_e
          .find("select")
          .html(
            '<option value="le">Within dates</option><option value="gt">Except dates</option>'
          );
        val_e.find("input").attr("placeholder", "Number of days");
        break;
      case "daterange":
        key_e.html(
          '<input type="text" readonly class="form-control-plaintext" value="Specific dates">'
        );
        cmp_e
          .find("select")
          .html(
      ...
      default:
        ...
        key_e
          .find("select")
          .prop("disabled", true)
          .html('<option value="">[ please select a filter type ]<option>');
        cmp_e
          .find("select")
          .prop("disabled", true)
          .html('<option value="">[ please select a filter type ]<option>');
        val_e
          .find("input")
          .prop("disabled", true)
          .attr("placeholder", "[ please select a filter type ]");
        break;
    }
  ...
  function add_filter_row(el) {
    var ftype_val = el
      .closest("tr")
      .find(".new-filter-type select")
      .val();
    var fkey_val = el
      .closest("tr")
      .find(".new-filter-key select")
      .val();
    var fcmp_val = el
      .closest("tr")
      .find(".new-filter-cmp select")
      .val();
    var fvalue_val = el
      .closest("tr")
      .find(".new-filter-value input")
      .val();
    var ftype_txt = el
      .closest("tr")
      .find(".new-filter-type option:selected")
      .text();
    var fkey_txt = el
      .closest("tr")
      .find(".new-filter-key option:selected")
      .text();
    var fcmp_txt = el
      .closest("tr")
      .find(".new-filter-cmp option:selected")
      .text();
    if (fkey_val == undefined) {
      fkey_val = fkey_txt = el
        .closest("tr")
        .find(".new-filter-key input")
        .val();
    }
    // Check that we have everything that we need
    if (ftype_val.trim() == "") {
      toastr.error("Please select a filter type");
      return;
    }
    if (fkey_val.trim() == "") {
      toastr.error("Please select a filter key");
      return;
    }
    if (fcmp_val.trim() == "") {
      toastr.error("Please select a comparison type");
      return;
    }
    if (fvalue_val.trim() == "") {
      toastr.error("Please select a filter value");
      return;
    }

    new_filter_row = $(
      "<tr>" +
        '<td><div class="new-filter-type" data-value="' +
        ftype_val +
        '">' +
        ftype_txt +
        "</div></td>" +
        '<td><div class="new-filter-key" data-value="' +
        fkey_val +
        '">' +
        fkey_txt +
        "</div></td>" +
        '<td><div class="new-filter-cmp" data-value="' +
        fcmp_val +
        '">' +
        fcmp_txt +
        "</div></td>" +
        '<td><div class="new-filter-value" data-value="' +
        fvalue_val +
        '"><code>' +
        fvalue_val +
        "</code></div></td>" +
    ...
    new_filter_row
      .appendTo(el.closest("table").find("tbody"))
      .find("td div")
      .hide()
      .slideDown();
    ...
    el.closest("tr")
      .find(".new-filter-key")
      .html(
        '<select disabled class="form-control"><option value="">[ please select a filter type ]<option></select>'
      );
    el.closest("tr")
      .find(".new-filter-cmp")
      .html(
    ...
    el.closest("tr")
      .find(".new-filter-value")
      .html(
        '<input disabled class="form-control" type="text" placeholder="[ please select a filter type ]">'
      );
    el.closest("tr")
      .find(".new-filter-add")
      .prop("disabled", true);
  ...
  $("body").on("click", ".new-filter-group-add-btn", function(e) {
    new_group_card = $(".new-filter-group-card")
      .first()
      .clone();
    ...
    window.ajax_update = $.ajax({
      ...
      success: function(data) {
        if (data["success"]) {
          ...
          $(".newfilter_num_filtered_samples").text(
            data["num_samples"] + " samples"
          );
          $(".newfilter_num_filtered_samples").removeClass(
          ...
          console.log(data);
          toastr.error(
            "There was an error applying the sample filters: " + data["message"]
          );
          $(".newfilter_num_filtered_samples")
  ...
  $("#filters_set")
    ...
    .change(function(e) {
      if ($(this).val() == "") {
        ...
        if (fs_name != null) {
          $("<option>" + fs_name + "</option>")
            .appendTo($("#filters_set"))
            .prop("selected", true);
    ...
    window.ajax_update = $.ajax({
      ...
      success: function(data) {
        ...
        // AJAX data['success'] was false
          ...
          console.log(data);
          toastr.error(
            "There was an error saving the sample filters:<br><em>" +
              data["message"] +
              "</em>"
          );
        }
  ...
  $(document).on("sample-filter-saved", function(e, fs) {
    ...
    var sfg = $("#sample-filter-group-select .nav-link").filter(function(
      index
    ) {
    ...
    if (sfg.length > 0) {
      sfg.addClass("active");
      $(
        '.sample-filters-group[data-filtergroup="' + fs["meta"]["set"] + '"]'
      ).show();
    } else {
      var idx = $("#sample-filter-group-select .nav-link").length + 1;
      $("#sample-filter-group-select").append(
        '<a class="nav-link active" href="#sample_filter_group_' +
          idx +
          '">' +
          fs["meta"]["set"] +
          "</a>"
      );
      $("#sample-filter-groups").append(
        '<div class="sample-filters-group" id="sample_filter_group_' +
          idx +
          '" data-filtergroup="' +
          fs["meta"]["set"] +
    ...
    $(
      '.sample-filters-group[data-filtergroup="' +
        fs["meta"]["set"] +
        '"] .list-group'
    ).append(
      '<button type="button" class="sample-filter-btn list-group-item list-group-item-action active" data-filterid="' +
        fs["filter_id"] +
        '">' +
        fs["meta"]["name"] +
        "</button>"
