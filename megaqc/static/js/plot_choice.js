    ...
    if ($(this).is(":checked")) {
      $.ajax({
        ...
        success: function(data) {
          for (key in data) {
            $("#sample_table tbody").append(
              "<tr class='sample_row_" +
                report_id +
                "'><td>" +
                key +
                "</td><td>" +
                data[key] +
                "</td><td><input type='checkbox' class='form-control sample_chbx' data-name='" +
                key +
                "'/></td></tr>"
            );
          }
...
function init_btns() {
  ...
  $(".plot_type_choice").each(function(idx) {
    $(this).click(function(e) {
      e.preventDefault();
      $("#plot_type_btn").html(
        $(this).data("type") + "<span class='caret'></span>"
      );
